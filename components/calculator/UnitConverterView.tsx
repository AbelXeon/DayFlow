...
import { unitCategories, convertLinear, convertTemperature } from "../../lib/units";
      ...
      <div className="rounded-2xl border border-border bg-surface p-5">
        ...
        <div className="flex items-center gap-2">
          <select
            ...
            className="flex-1 rounded-xl bg-surface-elevated border border-border px-3 py-3 text-sm text-text"
          >
            {unitKeys.map((u) => <option key={u}>{u}</option>)}
          </select>
          <button onClick={swap} className="rounded-xl bg-accent text-bg p-3 flex-shrink-0">
